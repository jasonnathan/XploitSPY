...
routes.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!username && !password) return res.redirect("/login?e=badLogin");
    if (!username) return res.redirect("/login?e=missingUsername");
    if (!password) return res.redirect("/login?e=missingPassword");
    const rUsername = db.maindb.get("admin.username").value();
    const rPassword = db.maindb.get("admin.password").value();
    ...
    if (
      ...
      return res.cookie("loginToken", loginToken).redirect("/panel");
    }
  })
);
